...
export function Docs({ path, signedIn }: { path: string; signedIn: boolean }) {
  ...
  useEffect(() => {
    if (md == null || !contentRef.current) {
      return;
    }
    const anchors =
      contentRef.current.querySelectorAll<HTMLAnchorElement>(".heading-anchor");
    ...
    <Window>
      ...
      <Card title="RAW MARKDOWN">
        <RowSpaceBetween className={styles.rawRow}>
          <span>
            ...
            or send <code>Accept: text/markdown</code>.
          </span>
          <div className={styles.rawButton}>
            <Button disabled={md == null} onClick={copyPage} theme="SECONDARY">
      ...
      <Card title="DOCS">
        {err ? (
          ...
          <div className={styles.docs} ref={contentRef}>
            {blocks.map((b, i) =>
              b.kind === "code" ? (
                <CodeBlock content={b.content} key={i} lang={b.lang} />
              ) : (
                <div
                  // biome-ignore lint/security/noDangerouslySetInnerHtml: Rendering sanitized markdown output from renderProse.
