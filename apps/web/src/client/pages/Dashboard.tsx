    ...
    <Window>
      <RowSpaceBetween style={{ marginBottom: "1rem" }}>
        ...
        <span className={styles.headerActions}>
          ...
          <button onClick={signOut} style={linkButtonStyle} type="button">
            sign out
          </button>
          <ThemeToggle />
        </span>
      ...
      <Card title={`SIGNED IN — ${status.email ?? "(unknown)"}`}>
        Your API keys authenticate requests to the gateway. One key per client;
        revoke any time.
      </Card>
          ...
          >
            ...
            >
              ...
              <div style={{ width: "18ch" }}>
                <Button onClick={copyNewKey} ref={copyButtonRef}>
      ...
      {revokingKey && (
        ...
        >
          ...
          <p style={{ opacity: 0.75, marginTop: "0.5rem" }}>
            Any client using this key will stop working. This cannot be undone.
          </p>
        </Dialog>
      )}
...
function extractLimits(data: unknown): Limit[] {
  ...
  if (limits.length === 0 && Array.isArray(d.rate_limits)) {
