          ...
          {rows.map((row, ri) => (
            <tr key={ri} tabIndex={0}>
              {row.map((cell, ci) => {
                ...
                if (STATUS_OK.has(cell)) {
                  statusClass = styles.statusOk;
                } else if (STATUS_OFF.has(cell)) {
                  statusClass = styles.statusOff;
                }
                const className =
                  [alignAt(ci), statusClass].filter(Boolean).join(" ") ||
                  undefined;
                return (
                  <td className={className} key={ci}>
                    {cell}
                  </td>
                );
              })}
